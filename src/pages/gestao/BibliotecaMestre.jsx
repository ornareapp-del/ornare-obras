...
const FASES = ['Pré-Montagem', 'Montagem', 'Pós-Montagem', 'Assistência Técnica', 'Garantia']
const AMBIENTES = ['Cozinha', 'Closet', 'Lavabo', 'Gourmet', 'Living', 'Suíte Master', 'Home Office', 'Lavanderia', 'Banheiro', 'Geral']
  ...
  async function salvarModelo() {
    ...
    if (error) {
      ...
      if (fallbackError) {
        ...
        setForm(novoForm())
        mostrarToast('Modelo criado com campos compatíveis do banco atual.')
        await carregar()
      }
    } else {
      setForm(novoForm())
...
.bm-kpi{background:#fff;border:1px solid ${THEME.border};border-top:3px solid ${THEME.gold};border-radius:14px;padding:16px 18px}
.bm-kpi.danger{border-top-color:${THEME.danger}}
.bm-kpi span{display:block;font-size:10px;letter-spacing:1.8px;text-transform:uppercase;color:${THEME.muted};font-weight:900;margin-bottom:9px}
.bm-kpi strong{display:block;font-size:32px;line-height:1;color:${THEME.ink}}
...
.bm-badge{display:inline-flex;border-radius:999px;background:#F6F1E8;color:${THEME.gold};font-size:10px;font-weight:900;padding:4px 8px}
.bm-badge.muted{color:${THEME.muted};background:#F7F4EF}
.bm-badge.danger{color:${THEME.danger};background:#FFF0F0}
...
@media (max-width:640px){.bm-page{padding:20px 14px 40px}.bm-header h1{font-size:32px}.bm-kpis{display:flex;overflow-x:auto;padding-bottom:5px}.bm-kpi{min-width:166px}.bm-card{padding:15px 13px;border-radius:15px}.bm-form-grid,.bm-filters{grid-template-columns:1fr}.bm-item{display:block}.bm-item button{margin-top:12px;width:100%}}
`
