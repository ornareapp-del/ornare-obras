...
export default function Equipe() {
  ...
  const [form, setForm] = useState({ full_name: '', email: '', role: 'montador', telefone: '' })
  ...
  async function salvar() {
    ...
    setForm({ full_name: '', email: '', role: 'montador', telefone: '' })
    setShowForm(false)
    await carregar()
    setSalvando(false)
  }
    ...
    <div style={{ padding: '40px 48px', maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--color-ink-muted)', marginTop: 6 }}>{profiles.length} membro{profiles.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {showForm && (
        <div style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: 24, marginBottom: 24 }}>
          <div style={{ fontSize: 9, letterSpacing: 2, color: 'var(--color-gold)', textTransform: 'uppercase', marginBottom: 16 }}>Novo Membro</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <div>
              <L>Nome *</L>
              <I value={form.full_name} onChange={v => setForm(p => ({ ...p, full_name: v }))} placeholder="Nome completo" />
            </div>
            <div>
              <L>E-mail *</L>
              <I type="email" value={form.email} onChange={v => setForm(p => ({ ...p, email: v }))} placeholder="email@exemplo.com" />
            </div>
            <div>
              <L>Telefone</L>
              <I value={form.telefone} onChange={v => setForm(p => ({ ...p, telefone: v }))} placeholder="(48) 99999-9999" />
            </div>
            <div>
              <L>Perfil</L>
        ...
        : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
            {lista.map(p => {
              ...
              return (
                <div key={p.id} style={{ background: '#fff', border: '1px solid var(--color-border)', borderRadius: 12, padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-ink)', marginBottom: 2 }}>{p.full_name || '—'}</div>
                    ...
                    <span style={{ fontSize: 10, padding: '2px 10px', borderRadius: 20, background: cor + '18', color: cor, fontWeight: 600 }}>
